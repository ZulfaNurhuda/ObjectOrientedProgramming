...
public abstract class AbstractAgency implements Publisher {
    ...
    public void addSubscriber(Subscriber s) {
        ...
        if (subscribers.contains(s)) {
            System.out.printf("Agensi %s melewatkan penambahan pelanggan karena sudah terdaftar.%n", agencyName);
        } else {
            subscribers.add(s);
            subscriberCategories.add(new ArrayList<>());
            System.out.printf("Agensi %s menambahkan pelanggan baru.%n", agencyName);
        }
    }
    ...
    public void removeSubscriber(Subscriber s) {
        ...
        int index = subscribers.indexOf(s); // Kalau hasilnya -1 artinya ga ditemuin s di subscribers
        if (index < 0) {
            System.out.printf("Agensi %s tidak menemukan pelanggan untuk dihapus.%n", agencyName);
        } else {
            subscribers.remove(index);
            subscriberCategories.remove(index);
            System.out.printf("Agensi %s menghapus pelanggan.%n", agencyName);
        }
    }
    ...
    public void broadcast(String message, String category) {
        ...
        for (int i = 0; i < subscribers.size(); i++) {
            ...
            if (categories.contains(category)) {
                subscriber.receive(agencyName, message, category);
            }
        }
    }
    ...
    public void publishUpdate(String message, String category) {
        ...
        System.out.printf("Agensi %s menyiarkan pembaruan kategori '%s': %s%n", agencyName, category, message);
