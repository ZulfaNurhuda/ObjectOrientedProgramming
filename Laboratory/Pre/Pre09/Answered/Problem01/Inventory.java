...
public class Inventory {
	...
	private String getKey(int row, int col) {
		...
		int colNum = col + 1;
		return String.format("%c%d", rowChar, colNum);
	}
	...
	public boolean addItem(String position, Item item) {
		...
		if (isValidKey(position) && grid.get(position) == null && currentWeight + itemWeight <= maxWeight) {
	...
	public Item removeItem(String position) {
		...
		if (isValidKey(position) && grid.get(position) != null) {
			removedItem = grid.get(position);
	...
	public boolean moveItem(String from, String to) {
		boolean success = false;
		if (isValidKey(from) && isValidKey(to) && grid.get(from) != null && grid.get(to) == null) {
			Item itemToMove = grid.get(from);
			grid.put(from, null);
			System.out.println("Removed " + itemToMove.getName() + " from position " + from);
			grid.put(to, itemToMove);
			System.out.println("Added " + itemToMove.getName() + " to position " + to);
			success = true;
		}
		return success;
	}
	...
	public Set<String> getOccupiedPositions() {
		...
		for (Map.Entry<String, Item> entry : grid.entrySet()) {
			if (entry.getValue() != null) {
				occupiedPositions.add(entry.getKey());
			}
		}
	...
	public Map<String, Item> searchByName(String name) {
		...
		for (Map.Entry<String, Item> entry : grid.entrySet()) {
			...
			if (item != null && item.getName().toLowerCase().contains(lowerName)) {
	...
	public Map<String, Item> filterByType(String type) {
		...
		for (Map.Entry<String, Item> entry : grid.entrySet()) {
			Item item = entry.getValue();
			if (item != null && item.getType().equalsIgnoreCase(type)) {
				result.put(entry.getKey(), item);
			}
		}
		return result;
	...
	public Map<String, Item> filterByWeightRange(double minWeight, double maxWeight) {
		...
		for (Map.Entry<String, Item> entry : grid.entrySet()) {
			Item item = entry.getValue();
			if (item != null && item.getWeight() >= minWeight && item.getWeight() <= maxWeight) {
				result.put(entry.getKey(), item);
			}
		}
		return result;
	...
	public List<Map.Entry<String, Item>> sortByName() {
		List<Map.Entry<String, Item>> itemList = new ArrayList<>();
		for (Map.Entry<String, Item> entry : grid.entrySet()) {
			if (entry.getValue() != null) {
				itemList.add(entry);
			}
		}
		Collections.sort(itemList, (e1, e2) -> e1.getValue().getName().compareToIgnoreCase(e2.getValue().getName()));
		return itemList;
	}
	...
	public List<Map.Entry<String, Item>> sortByRarity() {
		...
		Collections.sort(itemList, (e1, e2) -> Integer.compare(e2.getValue().getRarity(), e1.getValue().getRarity()));
		return itemList;
	}
	...
	public List<Map.Entry<String, Item>> sortByWeight() {
		...
		Collections.sort(itemList, (e1, e2) -> Double.compare(e1.getValue().getWeight(), e2.getValue().getWeight()));
		return itemList;
	}
	...
	public List<Map.Entry<String, Item>> sortByQuantity() {
		...
		Collections.sort(itemList, (e1, e2) -> Integer.compare(e2.getValue().getQuantity(), e1.getValue().getQuantity()));
		return itemList;
	}
	...
	public Map<String, Long> getTypeStatistics() {
		...
		for (Item item : getAllItems()) {
			...
			Long count = typeCount.containsKey(type) ? typeCount.get(type) + 1 : 1L;
	...
	public Map<Integer, Long> getRarityDistribution() {
		Map<Integer, Long> rarityCount = new LinkedHashMap<>();
		for (Item item : getAllItems()) {
			int rarity = item.getRarity();
			Long count = rarityCount.containsKey(rarity) ? rarityCount.get(rarity) + 1 : 1L;
	...
	public Map<String, Integer> getTotalQuantityByType() {
		Map<String, Integer> quantityByType = new LinkedHashMap<>();
		for (Item item : getAllItems()) {
			String type = item.getType();
			int totalQuantity = quantityByType.containsKey(type) ? quantityByType.get(type) + item.getQuantity() : item.getQuantity();
			quantityByType.put(type, totalQuantity);
		}
	...
	public List<Map.Entry<String, Item>> getHeaviestItems(int topN) {
		...
		Collections.sort(itemList, (e1, e2) -> Double.compare(e2.getValue().getWeight(), e1.getValue().getWeight()));
	...
	public List<Map.Entry<String, Item>> getLightestItems(int topN) {
		List<Map.Entry<String, Item>> itemList = sortByWeight();
		return itemList.subList(0, Math.min(topN, itemList.size()));
	}
	...
	public List<Map.Entry<String, Item>> getLegendaryItems() {
		List<Map.Entry<String, Item>> legendaryItems = new ArrayList<>();
		for (Map.Entry<String, Item> entry : grid.entrySet()) {
			Item item = entry.getValue();
	...
	public boolean hasItem(String itemName) {
		boolean found = false;
		List<Item> items = (List<Item>) getAllItems();
	...
	public double getAverageItemWeight() {
		...
		if (!items.isEmpty()) {
			...
			averageWeight = totalWeight / items.size();
		}
	...
	public double getAverageRarity() {
		...
		if (!items.isEmpty()) {
			...
			averageRarity = totalRarity / items.size();
		}
	...
	public void stackSimilarItems() {
		Map<String, List<Map.Entry<String, Item>>> itemGroups = new LinkedHashMap<>();
		...
		for (Map.Entry<String, Item> entry : grid.entrySet()) {
			...
			if (item != null) {
				...
				if (!itemGroups.containsKey(itemName)) {
					itemGroups.put(itemName, new ArrayList<>());
				}
				itemGroups.get(itemName).add(entry);
			}
		}

		for (Map.Entry<String, List<Map.Entry<String, Item>>> groupEntry : itemGroups.entrySet()) {
			...
			if (entries.size() > 1) {
				...
				for (int i = 1; i < entries.size(); i++) {
					String keyToClear = entries.get(i).getKey();
					grid.put(keyToClear, null);
				}
	...
	public String getFirstEmptySlot() {
		String firstEmptySlot = null;
		List<Map.Entry<String, Item>> entries = new ArrayList<>(grid.entrySet());
		for (int i = 0; i < entries.size() && firstEmptySlot == null; i++) {
			Map.Entry<String, Item> entry = entries.get(i);
			if (entry.getValue() == null) {
				firstEmptySlot = entry.getKey();
			}
		}
	...
	public boolean autoAddItem(Item item) {
		boolean success = false;
		String emptySlot = getFirstEmptySlot();
		if (emptySlot == null) {
			System.out.println("No empty slots available!");
		} else {
			success = addItem(emptySlot, item);
		}
		return success;
	}
	...
	public void displayGrid() {
		...
		System.out.println("Weight: " + String.format("%.1f", currentWeight) + "/" + String.format("%.1f", maxWeight) + " kg");
		System.out.println();
	...
	public void displayDetailedInventory() {
		...
		Collections.sort(items, (e1, e2) -> e1.getKey().compareToIgnoreCase(e2.getKey()));
